...
max_num_pl = 20                     # max number of total run samples possible
n_pl_min = num_pl                   # min number of converged run samples to provide results
burnin = 1. / 2.                    # Fraction of burn in period
efficiency_lim = [1, 80]            # limits of MH-MCMC acceptable efficiency

def loop(iterable_p):
    processor = Processor(model_params_estimate, model_type=model_type)
    for [n, loc_res] in iterable_p:
        dt0 = datetime.now()
        outfile_name = '%s_%s' % (loc_res['site_name'], loc_res['scale'])
        for pi in model_params_estimate:
            outfile_name = outfile_name+ '_' + pi
        outfile_name = outfile_name + '_' + str(n) + '.pickle' 
        ...
        p_ranges = {'s_wilt': [loc_res['s_h'], loc_res['s_fc']], # add ranges as necessary to optimize for other variables
        ...
        pl_results, n_it, fail_conv_count, fail_eff_count = processor.get_mcmc_mh_results(loc_res['s_obs'], theta0, p_ranges,
