...
class Inverse_bayesian_fitting(object):
    ...
    def init_random_model(self, params0_dict, maxcount=100):
        ...
        if while_count < 10:
            ...
            p0 = smpdf.get_p0()
            l = self.eval_loglikelihood(p0, self.s_obs, params)
    ...
    def test_model_consistency(self, params):
        ...
        if s_star < s_wilt or \
                ...
                s_fc < s_star or \
                s_wilt < s_h or \
                f_max > self.p_ranges['f_max'][1] or \
                f_max < self.p_ranges['f_max'][0] or \
                f_w > self.p_ranges['f_w'][1] or \
                f_w < self.p_ranges['f_w'][0] or \
                lneg > 0 or \
                lnan > 0:
            test = 0
        else:
    ...
    def get_mcmc_mh_results(self, s_obs, params_dict0, p_ranges,
        ...
        while (len(pl_results) < num_pl) and (it_count < max_num_pl):
            ...
            if len(pl_results) == num_pl:
                pl_results, it_conv = self.check_int_convergeance(pl_results, it_count)
                fail_conv_count = fail_conv_count + it_conv
